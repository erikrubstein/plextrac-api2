...
def list_client_users(session: AuthSession, tenant_id: int | str, client_id: int | str) -> list[ClientUser]:
...
def list_available_tenant_users(session: AuthSession, tenant_id: int | str, client_id: int | str) -> list[ClientUser]:
...
def remove_users_from_client(session: AuthSession, tenant_id: int | str, client_id: int | str, usernames: list[str]) -> OperationResult:
...
def list_tenant_clients(session: AuthSession, tenant_id: int | str) -> list[Client]:
...
def get_client(session: AuthSession, client_id: int | str) -> Client:
...
def delete_client(session: AuthSession, client_id: int | str) -> OperationResult:
    """Delete a client by client ID."""
    data = rest_request(session, "DELETE", f"/api/v1/client/{client_id}")
    return OperationResult.from_api(data if isinstance(data, dict) else {"data": data})


def add_client_logo(session: AuthSession, client_id: int | str, file: str | Path | BinaryIO, *, filename: str | None = None, content_type: str | None = None) -> OperationResult:
...
def delete_client_logo(session: AuthSession, client_id: int | str) -> OperationResult:
